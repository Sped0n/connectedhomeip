...
esp_err_t esp_openthread_init(const esp_openthread_platform_config_t * config)
{
    ...
    ESP_RETURN_ON_ERROR(esp_openthread_sleep_init(), OT_PLAT_LOG_TAG, "Failed to initialize OpenThread esp pm_lock");
#endif /* CONFIG_FREERTOS_USE_TICKLESS_IDLE && CONFIG_OPENTHREAD_RADIO_NATIVE */
    ESP_RETURN_ON_ERROR(esp_openthread_platform_init(config), OT_PLAT_LOG_TAG, "Failed to initialize OpenThread platform driver");
    esp_openthread_lock_acquire(portMAX_DELAY);
    ESP_RETURN_ON_FALSE(otInstanceInitSingle() != NULL, ESP_FAIL, OT_PLAT_LOG_TAG, "Failed to initialize OpenThread instance");
#if CONFIG_OPENTHREAD_DNS64_CLIENT && !CONFIG_CHIP_USE_OT_ENDPOINT
    ESP_RETURN_ON_ERROR(esp_openthread_dns64_client_init(), OT_PLAT_LOG_TAG, "Failed to initialize OpenThread dns64 client");
#endif
...
esp_err_t esp_openthread_auto_start(otOperationalDatasetTlvs * datasetTlvs)
{
    ...
    else
    {
        ...
        ESP_RETURN_ON_FALSE(otDatasetCreateNewNetwork(instance, &dataset) == OT_ERROR_NONE, ESP_FAIL, OT_PLAT_LOG_TAG,
                            "Failed to create OpenThread dataset");
#else
        ...
        // Extended Pan ID
        len =
            hex_string_to_binary(CONFIG_OPENTHREAD_NETWORK_EXTPANID, dataset.mExtendedPanId.m8, sizeof(dataset.mExtendedPanId.m8));
        ...
        // Network Key
        len = hex_string_to_binary(CONFIG_OPENTHREAD_NETWORK_MASTERKEY, dataset.mNetworkKey.m8, sizeof(dataset.mNetworkKey.m8));
...
esp_err_t esp_openthread_launch_mainloop(void)
{
    ...
    while (s_ot_mainloop_running)
    {
        ...
        if (select(mainloop.max_fd + 1, &mainloop.read_fds, &mainloop.write_fds, &mainloop.error_fds, &mainloop.timeout) >= 0)
        {
            esp_openthread_lock_acquire(portMAX_DELAY);
...
#if (CONFIG_OPENTHREAD_FTD || CONFIG_OPENTHREAD_MTD) && !CONFIG_CHIP_USE_OT_ENDPOINT
...
esp_err_t esp_openthread_start(const esp_openthread_config_t * config)
{
    ...
    assert(xTaskCreate(ot_task_worker, CONFIG_OPENTHREAD_TASK_NAME, CONFIG_OPENTHREAD_TASK_SIZE, &config,
                       CONFIG_OPENTHREAD_TASK_PRIORITY, &s_ot_task_handle) == pdPASS);
